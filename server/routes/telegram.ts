...
export const handleTelegramNotify: RequestHandler = async (req, res) => {
  try {
    ...
    const encodedToken = process.env.ENCODED_TOKEN || process.env.ENCODED_TELEGRAM_TOKEN;
    const encodedChatId = process.env.ENCODED_CHANNEL_ID || process.env.ENCODED_TELEGRAM_CHANNEL_ID;
    ...
    const caption = message || `${name || "Unknown"} - ${provider || "Provider"} added`;
    ...
    if (!tgResponse.ok || json?.ok === false) {
      return res.status(500).json({ success: false, error: "Failed to send Telegram message", details: json });
    }

    res.json({ success: true, result: json });
  } catch (err) {
    res.status(500).json({ success: false, error: "Unexpected error sending Telegram message" });
  }
};
